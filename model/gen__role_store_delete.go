...
func (r *RoleStoreImpl) Delete(ctx context.Context, id int64) error {
	...
	txNew = true
	qry := `
    DELETE FROM app_role obj WHERE
      obj.id = $1`
	slog.Debug("store.Role.Delete", "qry", qry, "id", id)
	res, err := tx.ExecContext(ctx, qry, id)
	if err != nil {
		return deletePostgresError(r.db, "store.Role.Delete", err,
			slog.String("qry", qry), "id", id,
		)
	}
	ra, err := res.RowsAffected()
	if err != nil {
		return deletePostgresError(r.db, "store.Role.Delete.RowsAffected", err,
			slog.String("qry", qry), "id", id,
		)
	}
	if ra == 0 {
